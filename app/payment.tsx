...
export default function PaymentScreen() {
  ...
  const handlePay = async () => {
    ...
    if (!deliveryInfo) {
      Alert.alert('Entrega não calculada', 'Por favor, aguarde o cálculo da entrega.');
      return;
    }

    if (paymentMethod === 'card' && (!cardName || !cardNumber || !cardExpiry || !cardCvv)) {
      Alert.alert('Dados incompletos', 'Por favor, preencha todos os dados do cartão.');
      return;
    }
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.section, { backgroundColor: theme.surface, borderColor: theme.border }]}>
        <Text style={[styles.sectionTitle, { color: theme.text }]}>Meu Carrinho</Text>
        {cart.map((item) => (
          <View key={item.cartId} style={styles.cartItem}>
            <Image source={item.image} style={styles.itemImage} resizeMode="contain" />
            <View style={{ flex: 1 }}>
              <Text style={[styles.itemName, { color: theme.text }]}>{item.nome}</Text>
              <Text style={{ color: theme.textSecondary, fontSize: 12 }}>{item.quantity}x</Text>
            </View>
            <Text style={[styles.itemPrice, { color: theme.primary }]}>{item.preco}</Text>
          </View>
        ))}
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.surface, borderColor: theme.border }]}>
        ...
        <TouchableOpacity
          style={[
            ...
            paymentMethod === 'card' && { borderColor: theme.primary, borderWidth: 2 },
            { borderColor: theme.border },
          ]}
          onPress={() => handlePaymentMethodChange('card')}
        >
          <Ionicons name="card" size={20} color={paymentMethod === 'card' ? theme.primary : theme.textSecondary} />
          <Text style={[styles.paymentLabel, { color: theme.text }]}>Cartão de Crédito</Text>
          <Ionicons name={paymentMethod === 'card' ? 'radio-button-on' : 'radio-button-off'} size={20} color={theme.primary} />
        </TouchableOpacity>

        {paymentMethod === 'card' && (
          <View style={styles.cardFields}>
            <TextInput
              style={[styles.input, { borderColor: theme.border, color: theme.text, marginBottom: 10 }]}
        ...
        <TouchableOpacity
          style={[
            styles.paymentOption,
            paymentMethod === 'pix' && { borderColor: theme.primary, borderWidth: 2 },
            { borderColor: theme.border },
          ]}
          onPress={() => handlePaymentMethodChange('pix')}
        >
          <Ionicons name="qr-code" size={20} color={paymentMethod === 'pix' ? theme.primary : theme.textSecondary} />
          <Text style={[styles.paymentLabel, { color: theme.text }]}>PIX</Text>
          <Ionicons name={paymentMethod === 'pix' ? 'radio-button-on' : 'radio-button-off'} size={20} color={theme.primary} />
        </TouchableOpacity>
        ...
        <TouchableOpacity
          style={[
            styles.paymentOption,
            paymentMethod === 'boleto' && { borderColor: theme.primary, borderWidth: 2 },
            { borderColor: theme.border },
          ]}
          onPress={() => {
        ...
        >
          <Ionicons name="document-text" size={20} color={paymentMethod === 'boleto' ? theme.primary : theme.textSecondary} />
          <Text style={[styles.paymentLabel, { color: theme.text }]}>Boleto</Text>
          <Ionicons name={paymentMethod === 'boleto' ? 'radio-button-on' : 'radio-button-off'} size={20} color={theme.primary} />
        </TouchableOpacity>

        {/* Exibição do QR Code PIX */}
        {paymentMethod === 'pix' && pixData && (
          <View style={[styles.pixContainer, { backgroundColor: theme.background, borderColor: theme.border }]}>
            <Text style={[styles.paymentLabel, { color: theme.text, marginBottom: 12 }]}>Chave PIX</Text>
            <View style={[styles.pixKeyBox, { backgroundColor: theme.surface, borderColor: theme.border }]}>
              <Text style={[styles.pixKey, { color: theme.text }]}>{pixData}</Text>
            </View>
            <TouchableOpacity
            ...
            >
              ...
              <Text style={{ color: '#FFF', fontWeight: '600' }}>Copiar Chave PIX</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {paymentMethod === 'boleto' && boletoData && (
          <View style={[styles.boletoContainer, { backgroundColor: theme.background, borderColor: theme.border }]}>
            <Text style={[styles.paymentLabel, { color: theme.text, marginBottom: 12 }]}>Código de Barras</Text>
            <View style={[styles.boletoCodeBox, { backgroundColor: theme.surface, borderColor: theme.border }]}>
              <Text style={[styles.boletoCode, { color: theme.text }]}>{boletoData}</Text>
            </View>
            <TouchableOpacity
              style={[styles.copyButton, { backgroundColor: theme.primary }]}
              onPress={() => {
                Alert.alert('Copiado', 'Código do boleto copiado para a área de transferência!');
              }}
            >
              <Ionicons name="copy" size={16} color="#FFF" style={{ marginRight: 8 }} />
              <Text style={{ color: '#FFF', fontWeight: '600' }}>Copiar Código</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.surface, borderColor: theme.border }]}>
        <View style={[styles.summaryRow, { borderBottomColor: theme.border }]}>
          ...
          <Text style={[styles.summaryValue, { color: theme.text }]}>R$ {subtotal.toFixed(2).replace('.', ',')}</Text>
        </View>
        <View style={[styles.summaryRow, { borderBottomColor: theme.border }]}>
          <Text style={{ color: theme.textSecondary }}>Entrega</Text>
          <Text style={[styles.summaryValue, { color: theme.text }]}>R$ {deliveryFee.toFixed(2).replace('.', ',')}</Text>
        </View>
        <View style={styles.summaryRow}>
          <Text style={[{ color: theme.text, fontWeight: '700' }]}>Total</Text>
          <Text style={[styles.totalValue, { color: theme.primary }]}>R$ {total.toFixed(2).replace('.', ',')}</Text>
        </View>
      </View>
      ...
      <TouchableOpacity
        ...
        disabled={processing}
      >
        <Text style={styles.payButtonText}>{processing ? 'Processando...' : 'Confirmar Pagamento'}</Text>
      </TouchableOpacity>

      <TouchableOpacity style={styles.backButton} onPress={() => router.back()}>
        <Text style={[{ color: theme.primary, textAlign: 'center', fontWeight: '600' }]}>Voltar</Text>
      </TouchableOpacity>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  paymentOption: { flexDirection: 'row', alignItems: 'center', padding: 12, borderRadius: 8, borderWidth: 1, marginBottom: 10, gap: 12 },
  paymentLabel: { flex: 1, fontSize: 14, fontWeight: '600' },
  cardFields: { marginTop: 12, paddingTop: 12, borderTopWidth: 1 },
  pixContainer: { marginTop: 16, padding: 16, borderRadius: 12, borderWidth: 1 },
  pixKeyBox: { padding: 12, borderRadius: 8, borderWidth: 1, marginBottom: 12 },
  pixKey: { fontSize: 12, fontWeight: 'bold', textAlign: 'center', fontFamily: 'monospace' },
  boletoContainer: { marginTop: 16, padding: 16, borderRadius: 12, borderWidth: 1 },
  boletoCodeBox: { padding: 12, borderRadius: 8, borderWidth: 1, marginBottom: 12 },
  boletoCode: { fontSize: 12, fontWeight: 'bold', textAlign: 'center', fontFamily: 'monospace', letterSpacing: 1 },
  copyButton: { flexDirection: 'row', paddingHorizontal: 16, paddingVertical: 12, borderRadius: 8, alignItems: 'center', justifyContent: 'center' },
  ...
  payButton: { height: 55, borderRadius: 12, justifyContent: 'center', alignItems: 'center', marginBottom: 12 },
